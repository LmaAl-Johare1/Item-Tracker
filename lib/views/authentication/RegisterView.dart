...
class _RegisterPageState extends State<RegisterPage> {
  ...
  void _handleFormSubmission() {
    if (!Validators.validateEmail(_emailController.text)) {
      _showSnackBar('Invalid email address');
      return;
    }

    if (!Validators.validatePassword(_passwordController.text)) {
      _showSnackBar('Password should be at least 6 characters long');
      return;
    }

    if (!Validators.validatePasswordMatch(
        _passwordController.text, _confirmPasswordController.text)) {
      _showSnackBar('Passwords do not match');
      return;
    }
    // if (!Validators.validatePhoneNumber(_phoneNumberController.text)) {
