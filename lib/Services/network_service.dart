...
class NetworkService {
  ...
  Future<void> deleteData(String collection, String field, String value) async {
    try {
      ...
      print('Document with ID: ${doc
          .id} deleted successfully from collection: $collection');
    } catch (e, stackTrace) {
  ...
  Future<List<Map<String, dynamic>>> fetchAll(String collection) async {
    try {
      ...
      final QuerySnapshot snapshot = await _firestore.collection(collection)
          .get();
      print('Fetched ${snapshot.docs
          .length} documents from collection: $collection');
  ...
  Future<void> updatePasswordInFirestore(String email, String newPassword) async {
    try {
      ...
      if (result.docs.isNotEmpty) {
        ...
        await _firestore.collection('Users').doc(doc.id).update({'password': newPassword});
